...
#set math.accent(dotless: false) // disables dotless i's and j's everywhere (namely in matrices and rows...)
...
A great number of elements described in the rest of the report heavily rely on the existing literature of pattern-matching analysis and compilation. We explicitly draw the line between the use of said references and our results.
...
$
  #text[*pattern*] p & := omega                                         &      #h(1cm) "wildcard" \
                     & #h(.5em) | C(p_1, dots.c, p_k) #h(.5cm) (k >= 0) & #h(2cm) "constructor" C \
...
Note that rows are allowed to be empty, and we will write them as unit rows: $unit$. We may also interpret singleton rows $row(tau) = mat(tau_1)$ simply as the inner type $tau_1$, so that we are able to give general definitions over patterns and rows simultaneously. A constructor pattern $A(p_1, ..., p_n)$ can thus be rewritten as $A(row(p))$, and so on. We will also write $omegas(n) = (omega, ..., omega)$ the row with $n$ wildcard patterns. Finally, we consider _matrices of patterns_, appearing with different notations.
$
...
*Remark.* Since we will be working with syntactic rules later on, we must disallow the same constructor to appear twice in $sig(tau)$ with different type arguments. Therefore, if $(A "of" row(tau_i)) in sig(tau)$, we will be able to destruct two constructor values $A(row(u_i))$ and $A(row(v_i))$ of type $tau$ with the guarantee that $row(u_i)$ and $row(v_i)$ are of the same type $row(tau_i)$. This restriction is similar to #ocaml in that we cannot declare an inductive type containing the same constructor twice.

#align(center)[
  ```ocaml
  # type a = Abc | Abc ;;
  (* Error: Two constructors are named Abc *)
  ```
]

It is important to note that in #ocaml, the only way that typing intervenes in pattern-matching analysis is when we need to extract a full signature from a type. Other than this, @MARANGET_2007 makes the assumption that every pattern has at least one matching value, despite the contradictory existence of the empty type (declared with the instruction #ml("type empty ;;")) which is explicitly ignored. We make an effort to bring back typing as part of pattern-matching semantics so that we are able to detect whenever a pattern-matching expression is trivially exhaustive.
...
#showybox[
  ...
  *$row(v) matches bold(P) : row(tau)$*
  ...
  $
    mat(v_1, dots.c, v_n) matches mat(p^((1))_1, dots.c, p^((1))_n; dots.v, dots.down, dots.v; p^((m))_1, dots.c, p^((m))_n) : mat(tau_1, dots.c, tau_n) #h(.2cm) <==> #h(.2cm) exists j <= m, space forall i <= n, space v_i matches p^((j))_i : tau_i
  $
]

*Examples*. Omitting the derivation trees, we are able to obtain the following conclusions:
$
  #ml("\"abc\"") & matches #ml("\"abc\"")                  && : #ml("string") \
  #ml("\"xyz\"") & matches omega                           && : #ml("string") \
      #ml("Nil") & matches #ml("Nil") or #ml("Cons") omega && : #ml("intlist") \
       #ml("42") & matches not #ml("1") and not #ml("2")   && : #ml("int") \
       mat(A, B) & matches mat(omega, A; A, omega)         && : (tau, tau) "   where" sig(tau) = {A, B}
$

Also, for any value $v$ of type $tau$, if we allow to see it as a pattern $v$, then we have $ v matches v : tau $

Again, if we restrict our language of patterns to wildwards, constructors and "or"-patterns, the definition is pretty much identical to the way it is presented in @MARANGET_2007, except that we consider both pattern-matching _and_ typing at the same time. This change may seem of little use at first, but it unlocks a useful way to talk about types and patterns with respect to pattern-matching analysis.
...
Algorithmically, appending "$and A(row(omega))$" to $p$ is not very interesting nor useful. Since we are iterating over every constructor $(A "of" row(tau_(1 i))) in sig(tau_1)$, we could instead attempt to define a more general operation mapping a matrix $bold(P)$ to a new matrix $bold(P) slash A$ satisfying the following property:
$
...
Specialization of a matrix is done by specializing each pattern in the column and stacking the resulting matrices vertically (in the sense of $or$). If $bold(P)$ is of width $n$ and $A$ of arity $k$, then $bold(P) slash A$ will be of width $k+n-1$.
$
...
Now, we obtain a more expressive decomposition theorem to partition the set of values matched by a matrix, restated below.

#showybox[
  *Theorem (signature decomposition)*
  For any type row $row(tau)$ with first component $tau_1$, the set of value rows matched by any matrix $bold(P)$ can be partitioned by ranging over every constructor $A$ in the signature of $tau_1$.
